<?php
//This was ripped from PHP-DI, opened an issue about making it it's own thing @ https://github.com/mnapoli/PHP-DI/issues/113
...
class PhpDocParser
{
	...
	public function getPropertyType(ReflectionClass $class, ReflectionProperty $property)
	{
		...
		if ($type[0] !== '\\') {
			...
			if (!$found) {
				throw new \Exception("The @var annotation on {$class->name}::" . $property->getName()
	...
	public function getParameterType(ReflectionClass $class, ReflectionMethod $method, ReflectionParameter $parameter)
	{
		...
		if ($type[0] !== '\\') {
			...
			if (!$found) {
				throw new \Exception("The @param annotation for parameter $parameterName of {$class->name}::" . $method->name
				. " contains a non existent class. Did you maybe forget to add a 'use' statement for this annotation?");
			}
		}
